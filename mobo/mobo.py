...
class MOBO:
    ...
    def solve(self, X_init, Y_init):
        ...
        for i in range(self.n_iter):
            ...
            # fit acquisition function
            self.acquisition.fit(X, Y)
            res = self.acquisition.final_batch()
            X_next = np.asarray([np.asarray(q) for q in res])


            # # This is for running reaction simulator using main.py
            # # Comment out the following lines if you want to use manually
            ...
            # This is for manual use using run_exp
            # Comment out the following lines if you want to use main.py
            print('Total evaluations: %d, hypervolume: %.4f\n' % (self.sample_num, self.status['hv']))
            yield X_next
