...
# from scikit_learn_old.sklearn.utils.optimize import _check_optimize_result
# from scikit_learn_old.sklearn.gaussian_process._gpr import GaussianProcessRegressor
# from scikit_learn_old.sklearn.gaussian_process.kernels import Matern, RBF, ConstantKernel
...
class GaussianProcess(SurrogateModel):
    ...
    def __init__(self, n_var, n_obj, nu, **kwargs):
        ...
        for _ in range(n_obj):
            ...
            gp = GaussianProcessRegressor(kernel=kernel, optimizer=constrained_optimization, random_state = 11)
            self.gps.append(gp)
    ...
    def evaluate(self, X, std=False, calc_gradient=False, calc_hessian=False):
        ...
        out = {'X': X, 'F': F, 'dF': dF, 'hF': hF, 'S': S, 'dS': dS, 'hS': hS}
        return out
