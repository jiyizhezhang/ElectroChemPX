...
def get_problem_options():
    problems = [
        ('reaction_conti', Reaction_conti),
        ('reaction_categ', Reaction_categ),
    ]
    return problems
...
def  build_problem(name, n_var, n_obj, n_init_sample, n_process=1):
    ...
    elif name.startswith('reaction_conti'):
        problem = get_problem(name, n_var=n_var, n_obj=n_obj)
    elif name.startswith('reaction_categ'):
        problem = get_problem(name, n_var=n_var, n_obj=n_obj)
    elif name.startswith('reaction_categ2'):
        problem = get_problem(name, n_var=n_var, n_obj=n_obj)
    elif name.startswith('reaction_real'):
        problem = get_problem(name, n_var=n_var, n_obj=n_obj)
    elif name.startswith('reaction_real2'):
        problem = get_problem(name, n_var=n_var, n_obj=n_obj)
    else:
        try:
